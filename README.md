# Self-cultivation of a tools expert / 工具达人的自我修养

---

## Content / 目录

---

- [Application](#Application)
    - [Developer_tool](#Developer_tool)
    - [iOS_app](#iOS_app)
    - [Windows_application](#Windows_application)
- [Extension](#Extension)
    - [Chrome_extension](#Chrome_extension)
    - [Firefox_addon](#Firefox_addon)
    - [IntelliJ_IDEA_plugin](#IntelliJ_IDEA_plugin)
- [Script](#Script)
    - [bat](#bat)
    - [ffmpeg](#ffmpeg)
    - [Tampermonkey](#Tampermonkey)
- [Shortcut](#Shortcut)
    - [Chrome](#Chrome)
    - [IntelliJ-IDEA](#IntelliJ-IDEA)
    - [Windows](#Windows)
    - [Word](#Word)
- [Software_Brand](#Software_Brand)
    - [JetBrains](#JetBrains)
    - [Bandisoft](#Bandisoft)
    - [Bandicam](#Bandicam)
    - [AntibodySoftware](#AntibodySoftware)
- [Website](#Website)

---

## Application

---

Recommend applications in most of all popular platform, including brief introduction in Chinese

---

### Developer_tool

| # | 名称 | 简介 | 推荐指数 |
|:---:|:---|:---|:---:|
| 00 | IntelliJ IDEA | 功能强大且体验友好的集成开发环境 | 9 * ⭐ |
| 01 | Visual Studio Code | 非常流行的代码编辑器 | 9 * ⭐ |
| 02 | Git | 最受欢迎的版本控制工具 | 9 * ⭐ |
| 03 | DataGrip | 功能强大的数据库客户端工具 | 9 * ⭐ |
| 04 | PyCharm | Python 集成开发环境 | 9 * ⭐ |
| 05 | GoLand | Golang 集成开发环境 | 9 * ⭐ |
|<img width=50px/>|<img width=400px/>|<img width=500px/>|<img width=100px/>|

### iOS_app

| # | 名称 | 简介 | 推荐指数 |
|:---:|:---|:---|:---:|
| 00 | Shortcuts-快捷指令 | 捷径 | 8 * ⭐ |
| 01 | ColorfulCloudsWeather-彩云天气 | 天气预报 | 9 * ⭐ |
| 02 | 计算器-多功能科学语音计算器 | 全能计算器 | 9 * ⭐ |
| 03 | GuideRank-盖得排行 | 消费品品牌排行榜 | 8 * ⭐ |
|<img width=50px/>|<img width=400px/>|<img width=500px/>|<img width=100px/>|

### Windows_application

| # | 名称 | 简介 | 推荐指数 |
|:---:|:---|:---|:---:|
| 00 | Firefox | 浏览器 | 9 * ⭐ |
| 01 | Ditto | 剪贴板增强工具 | 9 * ⭐ |
| 02 | Everything | 文件搜索器 | 9 * ⭐ |
| 03 | Snipaste | 截图工具 | 9 * ⭐ |
| 04 | Bandizip | 压缩软件 | 9 * ⭐ |
| 05 | Potplayer | 视频播放器 | 9 * ⭐ |
| 06 | WizTree | 磁盘空间分析工具 | 9 * ⭐ |
| 07 | Free Download Manager | 免费下载器 | 8 * ⭐ |
|<img width=50px/>|<img width=400px/>|<img width=500px/>|<img width=100px/>|

---

## Extension

---

Recommend extension for some amazing applications, just like Chrome, IntelliJ IDEA and so on

---

### Chrome_extension

| # | 名称 | 简介 | 推荐指数 |
|:---:|:---|:---|:---:|
| 00 | Tampermonkey | 油猴脚本 | 9 * ⭐ |
| 01 | uBlock Origin | 广告屏蔽插件 | 9 * ⭐ |
| 02 | 沙拉查词 | 单词翻译插件 | 8 * ⭐ |
| 03 | Chrono下载管理器 | 下载管理插件 | 8 * ⭐ |
| 04 | 为什么你们就是不能加个空格呢 | 中文页面浏览优化 | 8 * ⭐ |
| 05 | 购物党自动比价工具 | 商品历史价格查询 | 8 * ⭐ |
| 06 | Language Reactor | 字幕翻译插件 | 7 * ⭐ |
|<img width=50px/>|<img width=400px/>|<img width=500px/>|<img width=100px/>|

### Firefox_addon

| # | 名称 | 简介 | 推荐指数 |
|:---:|:---|:---|:---:|
| 00 | Tampermonkey | 油猴脚本 | 9 * ⭐ |
| 01 | uBlock Origin | 广告屏蔽插件 | 9 * ⭐ |
| 02 | Translate Web Pages | 网页翻译插件 | 9 * ⭐ |
| 03 | Download Manager (S3) | 下载管理插件 | 9 * ⭐ |
| 04 | 沙拉查词 | 单词翻译插件 | 8 * ⭐ |
| 05 | 购物党自动比价工具 | 商品历史价格查询 | 8 * ⭐ |
|<img width=50px/>|<img width=400px/>|<img width=500px/>|<img width=100px/>|

### IntelliJ_IDEA_plugin

| # | 名称 | 简介 | 推荐指数 |
|:---:|:---|:---|:---:|
| 00 | Save Actions | 保存时自动格式化 | 9 * ⭐ |
| 01 | Maven Helper | 依赖管理辅助插件 | 9 * ⭐ |
| 02 | Alibaba Java Coding Guidelines | 阿里 Java 编码指南 | 8 * ⭐ |
| 03 | google-java-format | 谷歌 Java 格式化插件 | 8 * ⭐ |
| 04 | String Manipulation | 字符串操作工具 | 8 * ⭐ |
| 05 | JavaDoc | Java 注释生成插件 | 8 * ⭐ |
| 06 | Json Parser | Json 格式化插件 | 8 * ⭐ |
| 07 | Lombok | 用注释取代样板代码 | 8 * ⭐ |
| 08 | Translation | 翻译工具 | 8 * ⭐ |
| 09 | CodeGlance | 代码地图插件 | 7 * ⭐ |
| 10 | Chinese (Simplified) Language Pack 中文语言包 | JetBrains 官方中文语言包 | 7 * ⭐ |
|<img width=50px/>|<img width=400px/>|<img width=500px/>|<img width=100px/>|

---

## Script

---

Some script to deal with repeat job in a simple way

### bat

[bat](https://github.com/bourneo/self-cultivation-of-a-tools-expert/blob/master/Script/bat)

### ffmpeg

[ffmpeg](https://github.com/bourneo/self-cultivation-of-a-tools-expert/blob/master/Script/ffmpeg/_ffmpeg.md)

### Tampermonkey

| # | 名称 | 简介 | 推荐指数 |
|:---:|:---|:---|:---:|
| 00 | HTML5视频播放器增强脚本 | 在浏览器里获得类似 Potplayer 体验的强大脚本 | 9 * ⭐ |
| 01 | 东方永页机 | 强大的自动翻页脚本 | 9 * ⭐ |
| 02 | Open the F--king URL Right Now | 自动跳转某些网站不希望用户直达的外链 | 8 * ⭐ |
| 03 | 片假名终结者 | 在网页中的日语外来语上方标注英文原词 | 8 * ⭐ |
| 04 | B站(Bilibili)搜索页面、个人主页已看视频标记 | 标记哪些视频是已经看过的 | 8 * ⭐ |
| 05 | Bilibili-Evolved | 强大的哔哩哔哩增强脚本 | 8 * ⭐ |
| 06 | 隐藏油管视频结尾的推荐 | 隐藏 Youtube 视频结尾遮挡画面的相关推荐图片 | 8 * ⭐ |
|<img width=50px/>|<img width=400px/>|<img width=500px/>|<img width=100px/>|

---

## Shortcut

---

Some script to deal with repeat job in a simple way

---

### Chrome

[Chrome-IDEA](https://github.com/bourneo/self-cultivation-of-a-tools-expert/blob/master/Shortcut/Chrome.md)

### IntelliJ-IDEA

[IntelliJ-IDEA](https://github.com/bourneo/self-cultivation-of-a-tools-expert/blob/master/Shortcut/IntelliJ-IDEA.md)

### Windows

[Windows](https://github.com/bourneo/self-cultivation-of-a-tools-expert/blob/master/Shortcut/Windows.md)

### Word

[Word](https://github.com/bourneo/self-cultivation-of-a-tools-expert/blob/master/Shortcut/Word.md)


---

## Software_Brand

---

### AntibodySoftware

- WizTree
- WizFile
- WizMouse

### Bandisoft

- Bandizip
- HoneyView
- Honeycam
- Shutdown8

### Bandicam

- Bandicam
- Bandicut

### JetBrains

- IntelliJ IDEA
- PyCharm
- GoLand
- WebStorm
- DataGrip

---

## Website

---

[Website](https://github.com/bourneo/self-cultivation-of-a-tools-expert/blob/master/Website/Website.md)

---




---

## Updating ...

--- 

